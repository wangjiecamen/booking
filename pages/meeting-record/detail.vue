<template>
    <view>
        <van-cell-group>
            ...
            <van-field label=" 备注" :value="formData.remark" type="textarea"></van-field>
            <van-cell v-if="formData._id" title="二维码">
                <template #right-icon>
                    <uqrcode style="display: inline-block;" ref="uqrcode" canvas-id="qrcode"
                        value="https://uqrcode.cn/doc" :size="100">
                    </uqrcode>
                </template>
            </van-cell>
            <van-cell title="会议纪要">
...
<script setup>
    ...
    const getDetail = async () => {
        const {
            data
        } = await uniCloud.importObject('booking').getItem(formData.value._id)
        console.log(data)
        formData.value = data
